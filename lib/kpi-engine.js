...
export async function recalculateDay(dispatchDate) {
  ...
  for (const [tourName, tour] of Object.entries(tourMap)) {
    ...
    for (const [ou, ouTasks] of Object.entries(ouGroups)) {
      const ratio   = ouTasks.length / totalTasks
      const ouDrops = Math.max(1, Math.round(uniqueDrops * ratio))
      const ouVol   = parseFloat((totalVol * ratio).toFixed(6))
      const ouWt    = parseFloat((totalWt  * ratio).toFixed(3))
