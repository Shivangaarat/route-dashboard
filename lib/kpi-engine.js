// lib/kpi-engine.js — v3 proportional OU split edition
...
export async function recalculateDay(dispatchDate) {
  ...
  // ── Build tour metrics with proportional OU split ─────────────────────────
  const tourMetrics = []
  ...
  for (const [tourName, tour] of Object.entries(tourMap)) {
    ...
    const isBulk     = routeType==='Single' && volUtil!==null && volUtil>=bulkMinUtil
    ...
    // ── Split by OU — unique drops and actual order metrics per OU ──────────
    const ouGroups = {}
    t.forEach(r => {
      const ou = (r.operating_unit || 'Unknown').trim()
      if (!ouGroups[ou]) ouGroups[ou] = { tasks: [], locations: new Set() }
      ouGroups[ou].tasks.push(r)
      if (r.location_id) ouGroups[ou].locations.add(r.location_id)
    })
    const isMultiOU = Object.keys(ouGroups).length > 1

    for (const [ou, ouGroup] of Object.entries(ouGroups)) {
      const ouTasks = ouGroup.tasks
      // Each OU gets its own unique drop count based on its orders' locations
      const ouDrops = ouGroup.locations.size || 1
      // Volume and weight are based on actual orders for this OU
      const ouVol   = parseFloat(ouTasks.reduce((s,r)=>s+(parseFloat(r.volume_cbm)||0),0).toFixed(6))
      const ouWt    = parseFloat(ouTasks.reduce((s,r)=>s+(parseFloat(r.weight_kg)||0),0).toFixed(3))
      const ouComp  = ouTasks.filter(r=>r.is_completed).length
      const ouFail  = ouTasks.filter(r=>r.is_failed).length
      const ouRejPct = ouTasks.length > 0 ? parseFloat((ouFail/ouTasks.length*100).toFixed(2)) : 0

      const ouPharmaOrders = analysisCategory==='HC' ? ouTasks.filter(r=>pharmaPatterns.some(p=>(r.category||'').toUpperCase().includes(p))).length : 0
      const ouMedlabOrders = analysisCategory==='HC' ? ouTasks.filter(r=>medlabPatterns.some(p=>(r.category||'').toUpperCase().includes(p))).length : 0
      const ouPharmaFailed = analysisCategory==='HC' ? ouTasks.filter(r=>r.is_failed&&pharmaPatterns.some(p=>(r.category||'').toUpperCase().includes(p))).length : 0
      const ouMedlabFailed = analysisCategory==='HC' ? ouTasks.filter(r=>r.is_failed&&medlabPatterns.some(p=>(r.category||'').toUpperCase().includes(p))).length : 0

      // For multi-OU tours, suffix the tour name with OU to keep unique constraint
      const metricTourName = isMultiOU ? `${tourName}__${ou}` : tourName

      tourMetrics.push({
        dispatch_date: dispatchDate, planned_tour_name: metricTourName,
        team, temp_category: tempCategory, analysis_category: analysisCategory,
        city, operating_unit: ou,
        ...
        total_orders: ouTasks.length, unique_drops: ouDrops,
        completed_orders: ouComp, failed_orders: ouFail,
        total_volume_cbm: ouVol, total_weight_kg: ouWt,
        volume_util_pct: volUtil, pallet_util_pct: null,
        route_type: routeType, is_bulk: isBulk, is_excluded: excluded,
        rejection_pct: ouRejPct,
        pharma_orders: ouPharmaOrders, medlab_orders: ouMedlabOrders,
        pharma_failed: ouPharmaFailed, medlab_failed: ouMedlabFailed,
      })
    }
  }
  ...
  for (let i=0; i<tourMetrics.length; i+=10) {
    ...
    await Promise.all(batch.map(m => sql`
      ...
      ON CONFLICT (dispatch_date, planned_tour_name) DO UPDATE SET
        ...
        rejection_pct=EXCLUDED.rejection_pct, operating_unit=EXCLUDED.operating_unit
    `))
  }
  ...
  return {
    date: dispatchDate,
    tours: Object.keys(tourMap).length,
